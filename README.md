...
* Googleカレンダーを定期的にチェックし、2分前に開始するGoogleMeet、Zoom、もしくはMicrosoft Teamsの会議のURLがある場合、そのURLをブラウザで開きます。
* 一度起動した会議は内部に記録しておき、再度起動しないようになっています。
* 「何分前にブラウザで起動するか」や「起動するブラウザ」はインストール時の設定で変更することができます。
* 0分、15分、30分、45分に開始する会議を対応しています。
...
* 条件２：会議の場所にURLが含まれる場合それを採用
* 条件３：本文にURLが含まれる場合それを採用

 ![image](https://github.com/fetaro/gcal-run/assets/1294741/7671f33b-760b-410c-916a-b76fa0e722a4)
